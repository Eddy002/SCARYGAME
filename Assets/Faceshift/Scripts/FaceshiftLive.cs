...
namespace fs
{
	public class FaceshiftLive : MonoBehaviour, NetworkConnectionCallback
	{
		...
		// base head position calculation variables
		private float timer = 0f;
		private float baseCalculationTime = 1f;
		private float baseX = 0f;
		private float baseY = 0f;
		private bool isBaseCalculated = false;
		private float ticksX = 0;
		private float ticksY = 0;
		...
		void OnResumeGame () 
		{ 
			//isBaseCalculated = false;
			//baseCalculationTime = 1f;
			//timer = 0f;
		}
		...
		public float getYHeadRotation ()
		{
			if (m_current_track != null) {
				if(isBaseCalculated)
				{
					return m_current_track.HeadRotation ().x - baseY;
				}
				else
				{
		...
		public float getXHeadRotation ()
		{
			if (m_current_track != null) {
				if(isBaseCalculated)
				{
					return m_current_track.HeadRotation ().y - baseX;
				}
				else
				{
		...
		{
			return newBlendShapeNamesArrived ();
		}
	}
}